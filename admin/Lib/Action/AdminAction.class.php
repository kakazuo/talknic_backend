...
class AdminAction extends BaseAction
{
	function index()
	{
		...
		$p = new Page($count,30);
		$admin_list = $admin_mod->field($prex.'admin.*,'.$prex.'role.name as role_name')->join('LEFT JOIN '.$prex.'role ON '.$prex.'admin.role_id = '.$prex.'role.id ')->limit($p->firstRow.','.$p->listRows)
				// ->order($prex.'admin.add_time DESC')
				->select();
		$key = 1;
